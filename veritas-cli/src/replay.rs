...
pub fn handle_replay(cache_root: Utf8PathBuf, args: ReplayArgs) {
    if let Some(rom_path) = args.n8auto {
        let n8map = FsMapper::load(cache_root.join("n8map.bin")).unwrap_or_else(|_| FsMapper::new());
        //n8map.pretty_print();
        let bundle = HashBundle::from_path(rom_path).unwrap().into_iter().next().unwrap();
